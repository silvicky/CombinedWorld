...
public class Silence
{
    ...
    private static int help(CommandSourceStack source)
    {
        ...
        source.sendSuccess(()-> Component.literal("/silence <dimension> <level>"),false);
        source.sendSuccess(()-> Component.literal("Get or set silence setting of dimension(s)."),false);
        return Command.SINGLE_SUCCESS;
    }
    private static int getDimensionLevel(CommandSourceStack source, ServerLevel dimension)
    {
        source.sendSuccess(()-> Component.literal(format("%06X",getServerState(source.getServer()).silence.getOrDefault(dimension.dimension().identifier(), 0)&0xFFFFFF)),false);
        return Command.SINGLE_SUCCESS;
    }
    private static int setDimensionLevel(CommandSourceStack source, ServerLevel dimension, int level)
    {
        getServerState(source.getServer()).silence.put(dimension.dimension().identifier(), level);
