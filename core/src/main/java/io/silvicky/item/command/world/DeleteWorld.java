...
public class DeleteWorld {
    ...
    private static int deleteWorld(CommandSourceStack source, Identifier idTmp, ServerLevel safeZone) throws CommandSyntaxException
    {
        ...
        new Thread(()->
        {
            try
            {
                if(newDimensions.containsKey(ResourceKey.create(Registries.LEVEL_STEM, id)))
                {
                    newDimensions.remove(ResourceKey.create(Registries.LEVEL_STEM, id));
                    if (!isSinglet)
                    {
                        newDimensions.remove(ResourceKey.create(Registries.LEVEL_STEM, idNether));
                        newDimensions.remove(ResourceKey.create(Registries.LEVEL_STEM, idEnd));
                    }
                }
                try
                {
                ...
                try
                {
                    ...
                    for (String i : removedPlayers)
                        stateSaver.savedMap.getOrDefault(id.getNamespace(), new HashMap<>()).remove(i);
